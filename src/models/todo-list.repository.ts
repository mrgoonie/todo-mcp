...
export class TodoListRepository {
	...
	async create(data: Omit<TodoList, "id" | "created_at" | "updated_at">): Promise<TodoList> {
		...
		await this.db.run(
			`INSERT INTO todo_lists (id, name, description, metadata, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?)`,
			[
				id,
				data.name,
	...
	async findById(id: string): Promise<TodoList | null> {
		const row = await this.db.get<any>("SELECT * FROM todo_lists WHERE id = ?", [id]);
	...
	async update(
		id: string,
		data: Partial<Omit<TodoList, "id" | "created_at" | "updated_at">>
		...
		values.push(id);
		await this.db.run(`UPDATE todo_lists SET ${updates.join(", ")} WHERE id = ?`, values);
